...
use Api\AppService\AutoBillApiAuthException;
use Api\ApiHelper\Constant\AutoBillConstant;

class AutoBillApiResponseUtil
{
    ...
    {
        return array(
            AutoBillConstant::IS_SUCCESS => $isSuccess,
            AutoBillConstant::STATUS_CODE => $statusCode,
            "$responseKey" => $data
        );
    }
    ...
    public static function sentException($message, $httpCode, $systemCode, $isSuccess)
    {
        ...
        return new AutoBillApiAuthException($message, $httpCode, $apiResponse);
    }
